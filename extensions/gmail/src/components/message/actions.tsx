  ...
  return (
    <ActionPanel.Submenu title={props.title} shortcut={props.shortcut} icon={Icon.Book}>
      <ActionPanel.Submenu title="Match">
        {labelsAll.map((l) => (
          <Action key={`match-${l.id}`} title={getLabelName(l) || "?"} icon={getLabelIcon(l)} onAction={() => handle(l, "")} />
        ))}
      </ActionPanel.Submenu>
      <ActionPanel.Submenu title="Not Match">
        {labelsAll.map((l) => (
          <Action key={`not-match-${l.id}`} title={getLabelName(l) || "?"} icon={getLabelIcon(l)} onAction={() => handle(l, "-")} />
        ))}
      </ActionPanel.Submenu>
    </ActionPanel.Submenu>
  );
